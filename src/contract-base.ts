...
import {rpc, tx, u, wallet, api} from '@cityofzion/neon-js';
import {rpc as n3rpc, sc} from '@cityofzion/neon-core';
...
export class SeraphIDContractBase {
  ...
  protected async sendSignedTransaction(
    ...
    const { feePerByte, executionFeeFactor } = await api.getFeeInformation(client);

    transaction.networkFee =  api.calculateNetworkFee(
      transaction,
      feePerByte,
      executionFeeFactor
    );
   ...
   */
  protected extractResult(res: n3rpc.InvokeResult): IResult {
